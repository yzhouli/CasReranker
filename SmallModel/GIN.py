...
class BaseGNN(nn.Module):
    def __init__(self, hidden_channels):
        super().__init__()
        # GIN 核心：必须在卷积内部定义一个 MLP 来提升非线性表达能力
        # 开启 train_eps=True 让中心节点权重动态可学习
        mlp1 = Sequential(
...
class DiffusionReRanker(nn.Module):
    def __init__(self, num_users, num_news, hidden_channels, metadata):
        ...
        # 理论上 GIN 必须使用 'sum' 聚合才能保证 WL 同构区分能力
        # 所以这里将异构聚合方式由 'mean' 改为 'sum'
        self.gnn = to_hetero(BaseGNN(hidden_channels), metadata=metadata, aggr='sum')
...
def evaluate_reranking_with_pkl(model, data, dataset_path, user_map, news_map):
    ...
    with torch.no_grad():
        ...
        pdbr = tqdm(test_db, desc='GIN Eval')
        for item in pdbr:
            ...
            if len(valid_hist_embs) > 0:
                # GIN 基线使用了 sum 聚合，在冷启动推断时为了数值稳定，可以保持平均
