...
# ==========================================
# 3. 静态图构建 (新增 exclude_news_set 防止数据泄露)
# ==========================================
def build_hetero_graph(file_path, exclude_news_set=None):
    """从 cascades.txt 构建异构图，严格过滤测试集的 news_id"""
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # 核心拦截逻辑：如果该 news 属于测试集，直接跳过整条级联不参与建图！
...
def evaluate_reranking_with_pkl(model, data, dataset_path, user_map, news_map):
    ...
    with torch.no_grad():
        ...
        pdbr = tqdm(test_db, desc='GNN Eval')
        for item in pdbr:
            ...
            # 因为我们在训练时剔除了 test news，所以这里一定找不到 news_id。
            # 我们通过聚合该 news 的历史传播者 (history_users) 的特征，实时生成这个未知 news 的 Embedding
            valid_hist_embs = []
...
if __name__ == '__main__':
    ...
    # 1. 提前读取 test.pkl，获取需要剔除的 news_id 集合
    ...
    # 2. 传入 exclude_news_set 构建“干净”的训练图
    print("\n2. 加载级联数据并构建全局图结构...")
