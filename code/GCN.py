...
class BaseGNN(nn.Module):
    def __init__(self, hidden_channels):
        super().__init__()
        # 修复：使用 GraphConv，原生支持异构图/二分图的 Tuple 输入
        self.conv1 = GraphConv(-1, hidden_channels, aggr='mean')
        self.conv2 = GraphConv(-1, hidden_channels, aggr='mean')
...
def evaluate_reranking_with_pkl(model, data, dataset_path, user_map, news_map):
    ...
    with torch.no_grad():
        ...
        pdbr = tqdm(test_db, desc='GraphConv Eval')
        for item in pdbr:
