...
# 核心修改：引入原生异构图 Transformer 卷积层 HGTConv
...
class DiffusionReRanker(nn.Module):
    def __init__(self, num_users, num_news, hidden_channels, metadata):
        ...
        # 核心修改：使用原生 HGTConv。需要传入 metadata，并设置多头注意力 heads
        num_heads = 4
    ...
    def forward(self, data):
        ...
        # HGT 属于 Transformer 变种，通常配合 GELU 激活函数使用
...
def evaluate_reranking_with_pkl(model, data, dataset_path, user_map, news_map):
    ...
    with torch.no_grad():
        ...
        for item in test_db:
            ...
            candidate_users = [str(u).strip() for u in item["neg_users"][:49]]
...
if __name__ == '__main__':
    ...
    print("\n3. 初始化并训练 HGT 模型 (BPR Loss)...")
    hidden_dim = 64
    # HGT 必须传入 data.metadata() 以识别异构节点和边缘类型
    model = DiffusionReRanker(data['user'].num_nodes, data['news'].num_nodes, hidden_dim, data.metadata()).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)

    train_model(model, data, optimizer, dataset_path=dataset_path, user_map=user_map, news_map=news_map, epochs=3000)
